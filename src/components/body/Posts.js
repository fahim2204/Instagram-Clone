...
const Posts = (props) => {
    ...
    return (
        <>
            ...
            {/* Comment View Modal >> S */}
            <div className="modal fade" id="addNewModal" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="AddNewModal" aria-hidden="true">
                <div className="modal-dialog modal-lg modal-fullscreen-md-down modal-dialog-centered">
                    <div className="modal-content rounded-0">
                        <div className="modal-body p-0">
                            {currentPost && <Comments post={currentPost} timeDifference={timeDifference} handleLike={handleLike}/>}
                        </div>
                    </div>
                </div>
            </div>
            {/* Comment View Modal >> E  */}
            {
                props.dummyApi.map((item, index) => {
                    return (
                        <div key={index} className="row bg-white rounded-2 border my-3 pb-2">
                            <div className="col-12 d-flex justify-content-between align-items-center my-1 ms-0 ps-0">
                            ...
                            <div className="col-12 d-flex justify-content-between align-content-center pt-2 pe-3">
                                <div>
                                    ...
                                    <span className="hover-opacity" type="button" data-bs-toggle="modal" data-bs-target="#addNewModal" onClick={()=>setCurrentPost(item)}>
                            ...
                            <div className="col-12 pt-2 mx-2 d-flex flex-column">
                                <span className="text-muted" type="button" data-bs-toggle="modal" data-bs-target="#addNewModal" onClick={()=>setCurrentPost(item)}><small>{item.comments ? `View all ${item.comments.length} comments` : "No Comments"}</small></span>
